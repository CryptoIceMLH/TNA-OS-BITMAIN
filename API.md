...
**Firmware:** v0.11.0 · **Updated:** 2026-07-29 · **Applies to:** Antminer S19 / S21 series (all A113D Amlogic control boards)
...
| POST | `/api/wifi/configure` | Save WiFi creds → reboot into station mode (USB WiFi dongle) |
...
### 3.11a `POST /api/wifi/configure` — WiFi provisioning (optional USB dongle)

TNA-OS supports an **optional USB WiFi dongle** on the control-board USB port, so a miner can run on
**Ethernet _or_ WiFi**. Ethernet always wins when a cable is connected; WiFi is used only when
there's no Ethernet. Units with no dongle are unaffected — all WiFi fields read `0`/`""`.

**Supported chipsets** (Realtek USB): `RTL8723DU`, `RTL8821CU`, `RTL8811CU`, `RTL8822BU`,
`RTL8812BU`, `RTL8812AU`, `RTL8811AU` (plus in-kernel `rtl8xxxu` for older RTL8188/8192).

**Provisioning flow:**
1. Plug the dongle in. With **no saved credentials and no Ethernet**, the miner raises a setup
...
|---|---|---|
| `networkMode` | `"ethernet"` / `"wifi"` / `"ap"` | current uplink; `ap` = setup hotspot |
| `apMode` | bool | true while running the `TNA-Setup` hotspot |
| `apSsid` / `apIp` | `"TNA-Setup"` / `"192.168.4.1"` | set only in AP mode, else `""` |
| `ssid` | string | joined network's SSID (station mode) |
| `wifiChipset` | string | bound dongle chipset (cosmetic) |
| `wifiStatus` | `"ap"` / `"connected"` / `""` | radio state |
...
has a real uplink IP — Ethernet leased **or** WiFi joined (the setup AP alone does not count). It
won't burn power hashing with no reachable pool, and starts mining automatically the moment a
network comes online. See `GET /api/power/state`.
